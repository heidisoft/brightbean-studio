...
class ComposerQueueNextAvailableTests(TestCase):
    """Composer next-available scheduling should stay channel-specific."""
    ...
    def setUp(self):
        ...
        self.org = Organization.objects.create(name="Test Org", default_timezone="UTC")
        self.workspace = Workspace.objects.create(organization=self.org, name="Test Workspace")
        OrgMembership.objects.create(
            user=self.user,
            organization=self.org,
    ...
    def _slot_datetime(self, slot_date, slot_time):
        return datetime.combine(slot_date, slot_time, tzinfo=ZoneInfo("UTC"))

    def test_next_available_with_floor_date_updates_matching_channel_queue_entries(self):
        floor_date = timezone.localdate() + timedelta(days=3)
        account_a_first = self._slot_datetime(floor_date, time(9, 0))
        account_a_second = self._slot_datetime(floor_date + timedelta(days=1), time(9, 0))
        account_b_first = self._slot_datetime(floor_date, time(14, 0))

        PostingSlot.objects.create(
            social_account=self.account_a,
            day_of_week=floor_date.weekday(),
            time=time(9, 0),
        )
        PostingSlot.objects.create(
            social_account=self.account_a,
            day_of_week=(floor_date + timedelta(days=1)).weekday(),
            time=time(9, 0),
        )
        PostingSlot.objects.create(
            social_account=self.account_b,
        ...
        response = self.client.post(
            ...
            data={
                ...
                "caption": "Queue me",
                "tags": "",
                "selected_accounts": f"{self.account_a.id},{self.account_b.id}",
                "scheduled_date": floor_date.isoformat(),
            },
        )

        self.assertIn(response.status_code, (200, 204, 302))
        post = Post.objects.filter(workspace=self.workspace, caption="Queue me").latest("created_at")
        page_a_post = post.platform_posts.get(social_account=self.account_a)
        page_b_post = post.platform_posts.get(social_account=self.account_b)
        self.assertEqual(page_a_post.scheduled_at, account_a_second)
        self.assertEqual(page_b_post.scheduled_at, account_b_first)
        self.assertEqual(post.queue_entries.get(queue=self.queue_a).assigned_slot_datetime, account_a_second)
        self.assertEqual(post.queue_entries.get(queue=self.queue_b).assigned_slot_datetime, account_b_first)
